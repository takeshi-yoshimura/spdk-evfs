...
DEPS_PMDK=-lspdk_bdev_pmem
DEPS_PMDK2="-L/home/tyos/data/src/pmdk/src/nondebug -lpmemblk -lpmem"
...
DEPS_DPDK="-Wl,--version-script,$cur/versions.ldscript -Wl,--whole-archive -Wl,-Bstatic -L$NUMADIR -lnuma $DEPS_PMDK2 -Wl,-Bdynamic -Wl,--no-whole-archive -Wl,--start-group -Wl,--whole-archive -L$DPDKDIR -lrte_eal -lrte_mempool -lrte_ring -lrte_mempool_ring -lrte_pci -lrte_bus_pci -Wl,--end-group -Wl,--no-whole-archive"
...
cmd="gcc -fPIC -shared -o $OUTDIR/libposixfs.so $DEPS_DPDK -Wl,--whole-archive -L$SPDKDIR $DEPS_SPDK $DEPS_CAPI $DEPS_PMDK -lspdk_bdev_nvme -lspdk_blobfs -lspdk_posixfs -Wl,--no-whole-archive $DEPS_CAPI2 -luuid -lrt -lpthread -ldl"
echo $cmd
$cmd
